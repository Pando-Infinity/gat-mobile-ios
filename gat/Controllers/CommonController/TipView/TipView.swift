...
class EasyTipView: UIView {
    ...
    fileprivate(set) var focus: UIView
    fileprivate(set) open var preferences: Preferences
    fileprivate var arrowTip: CGPoint
    fileprivate let content: Content
    
    var tipFrame: CGRect = .zero
    
    fileprivate var clipPath: UIBezierPath
    
    override open var backgroundColor: UIColor? {
        didSet {
            guard let color = self.backgroundColor, color != .clear else { return }
    ...
    fileprivate lazy var contentSize: CGSize = { [unowned self] in
        switch self.content {
        case .attributed(let attrs):
            var textSize = attrs.boundingRect(with: .init(width: self.preferences.positioning.maxWidth, height: .greatestFiniteMagnitude), options: .usesLineFragmentOrigin, context: nil).size
            ...
            return textSize
        case .view(let v): return v.frame.size
        }
    }()
    ...
    fileprivate init(content: Content, clipPath: UIBezierPath, forcus: UIView, preferences: Preferences, delegate: EasyTipViewDelegate?) {
        ...
        self.backgroundColor = preferences.drawing.backgroundColor
    }
    
    convenience init(attributed: NSAttributedString, clipPath: UIBezierPath, forcus: UIView, preferences: Preferences, delegate: EasyTipViewDelegate?) {
        self.init(content: .attributed(attributed), clipPath: clipPath, forcus: forcus, preferences: preferences, delegate: delegate)
    }
    
    convenience init(view: UIView, forcus: UIView, clipPath: UIBezierPath, preferences: Preferences, delegate: EasyTipViewDelegate?) {
        self.init(content: .view(view), clipPath: clipPath, forcus: forcus, preferences: preferences, delegate: delegate)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let view = self.focus.superview {
            let location = touch.location(in: view)
            self.dismiss(focus: self.focus.frame.contains(location))
        } else {
...
extension EasyTipView {
    ...
    fileprivate func drawOverlay() {
        ...
        overlayLayer.fillColor = self.preferences.drawing.backgroundColor.cgColor
        self.layer.addSublayer(overlayLayer)
        let path = CGMutablePath()
        path.addRect(self.frame)
        path.addPath(self.clipPath.cgPath)
        path.closeSubpath()
    ...
    fileprivate func drawBubble() {
        ...
        contourPath.addRoundedRect(in: self.tipFrame, cornerWidth: self.preferences.drawing.cornerRadius, cornerHeight: self.preferences.drawing.cornerRadius)
        
        contourPath.closeSubpath()
        
        let layer = CAShapeLayer()
        layer.fillColor = self.preferences.drawing.backgroundColorTip.cgColor
        layer.path = contourPath
        if self.preferences.hasBorder {
            self.drawBorderLayer(layer)
        }
        if self.preferences.hasShadow {
            self.drawShadow(layer: layer)
        }
        ...
        case .attributed(let attributed):
            ...
            label.frame = .init(origin: self.tipFrame.origin + CGPoint(x: (self.tipFrame.width - self.contentSize.width) / 2.0, y:  (self.tipFrame.height - self.contentSize.height) / 2.0), size: self.contentSize)
            self.addSubview(label)
        case .view(let view):
            view.frame = self.tipFrame
            self.addSubview(view)
            view.layoutIfNeeded()
        }
    }
    
    fileprivate func drawBorderLayer(_ layer: CALayer) {
        layer.borderColor = self.preferences.drawing.borderColor.cgColor
        layer.borderWidth = self.preferences.drawing.borderWidth
    }
    
    fileprivate func drawShadow(layer: CALayer) {
        layer.masksToBounds = false
        layer.shadowColor = self.preferences.drawing.shadowColor.cgColor
        layer.shadowOffset = self.preferences.drawing.shadowOffset
        layer.shadowRadius = self.preferences.drawing.shadowRadius
        layer.shadowOpacity = Float(self.preferences.drawing.shadowOpacity)
    }
}

extension EasyTipView {
    func dismiss(focus: Bool, withCompletion completion: (() -> ())? = nil){
        if focus {
            let damping = self.preferences.animating.springDamping
            let velocity = self.preferences.animating.springVelocity
            UIView.animate(withDuration: preferences.animating.dismissDuration, delay: 0, usingSpringWithDamping: damping, initialSpringVelocity: velocity, options: [.curveEaseInOut], animations: {
                ...
                self.delegate?.easyTipViewDidDismiss(self, forcus: focus)
                completion?()
                self.removeFromSuperview()
                self.transform = CGAffineTransform.identity
            }
        } else {
            self.delegate?.easyTipViewDidDismiss(self, forcus: false)
        }
        
    }
    
    @objc func handleTap(gestures: UITapGestureRecognizer) {
        if let view = self.focus.superview {
            let location = gestures.location(in: view)
            self.dismiss(focus: self.focus.frame.contains(location))
        } else {
            self.dismiss(focus: false)
        }
    ...
    func show(animated: Bool = true, withinSuperview superview: UIView) {
        
        let initialTransform = self.preferences.animating.showInitialTransform
        let finalTransform = self.preferences.animating.showFinalTransform
        let initialAlpha = self.preferences.animating.showInitialAlpha
        let damping = self.preferences.animating.springDamping
        let velocity = self.preferences.animating.springVelocity
...
//        let tap = UITapGestureRecognizer(target: self, action: #selector(handleTap(gestures:)))
//        tap.delegate = self
//        self.addGestureRecognizer(tap)
        ...
        if animated {
            UIView.animate(withDuration: self.preferences.animating.showDuration, delay: 0, usingSpringWithDamping: damping, initialSpringVelocity: velocity, options: [.curveEaseInOut], animations: animations, completion: nil)
        } else {
            animations()
        }
    }
...
extension EasyTipView {
    fileprivate func arrange(withinSuperview superview: UIView) {
        var position = self.preferences.drawing.arrowPosition
        let refViewFrame = self.focus.superview!.convert(self.focus.frame, to: superview)
        ...
        var frame = self.computeFrame(arrowPosition: position, refViewFrame: refViewFrame, superviewFrame: superviewFrame)
        if !self.isFrameValid(frame, forRefViewFrame: refViewFrame, withinSuperviewFrame: superviewFrame) {
            for value in ArrowPosition.allCases where value != position {
                let newFrame = self.computeFrame(arrowPosition: value, refViewFrame: refViewFrame, superviewFrame: superviewFrame)
        ...
        switch position {
        case .bottom, .top, .any:
            frame.origin.y += position == .bottom ? -self.preferences.drawing.arrowHeight : self.preferences.drawing.arrowHeight
        case .right, .left:
            frame.origin.x += position == .left ? -self.preferences.drawing.arrowWidth : self.preferences.drawing.arrowWidth
        }
    ...
    fileprivate func computeFrame(arrowPosition position: ArrowPosition, refViewFrame: CGRect, superviewFrame: CGRect) -> CGRect {
        ...
        switch position {
        case .top, .any:
            xOrigin = refViewFrame.center.x - self.tipViewSize.width / 2
            yOrigin = refViewFrame.origin.y + refViewFrame.height
        case .bottom:
            xOrigin = refViewFrame.center.x - self.tipViewSize.width / 2
            yOrigin = refViewFrame.origin.y - self.tipViewSize.height
        case .right:
            xOrigin = refViewFrame.origin.x - self.tipViewSize.width
            yOrigin = refViewFrame.center.y - self.tipViewSize.height / 2
        case .left:
            xOrigin = refViewFrame.origin.x + refViewFrame.width
            yOrigin = refViewFrame.origin.y - self.tipViewSize.height / 2
        }
        
        var frame = CGRect(x: xOrigin, y: yOrigin, width: self.tipViewSize.width, height: self.tipViewSize.height)
    ...
    fileprivate func getContentRect(from bubbleFrame: CGRect) -> CGRect {
        return CGRect(x: bubbleFrame.origin.x + (bubbleFrame.size.width - self.contentSize.width) / 2, y: bubbleFrame.origin.y + (bubbleFrame.size.height - contentSize.height) / 2, width: self.contentSize.width, height: self.contentSize.height)
    }
}

extension EasyTipView {
    fileprivate func getBubbleFrame() -> CGRect {
        ...
        case .bottom, .top, .any:
            ...
            bubbleWidth = self.tipViewSize.width - 2 * self.preferences.positioning.bubbleHInset
            bubbleHeight = tipViewSize.height - 2 * self.preferences.positioning.bubbleVInset - self.preferences.drawing.arrowHeight
            
            bubbleXOrigin = self.preferences.positioning.bubbleHInset
            bubbleYOrigin = arrowPosition == .bottom ? self.preferences.positioning.bubbleVInset : self.preferences.positioning.bubbleVInset + self.preferences.drawing.arrowHeight
            
        case .left, .right:
            
            bubbleWidth = self.tipViewSize.width - 2 * self.preferences.positioning.bubbleHInset - self.preferences.drawing.arrowHeight
            bubbleHeight = self.tipViewSize.height - 2 * self.preferences.positioning.bubbleVInset
            
            bubbleXOrigin = arrowPosition == .right ? self.preferences.positioning.bubbleHInset : self.preferences.positioning.bubbleHInset + self.preferences.drawing.arrowHeight
...
extension EasyTipView {
    enum ArrowPosition: CaseIterable {
        case any
        case top
        case bottom
        case right
...
extension EasyTipView {
    struct Preferences {
        struct Drawing {
            ...
            var backgroundColorTip  = UIColor.blue
        ...
        var hasBorder : Bool { self.drawing.borderWidth > 0 && self.drawing.borderColor != .clear }
        
        var hasShadow : Bool { self.drawing.shadowOpacity > 0 && self.drawing.shadowColor != .clear }
    }
}
...
public protocol TipViewDelegate : class {
    func easyTipViewDidDismiss(_ tipView : TipView)
}


// MARK: - Public methods extension
public extension TipView {
     ...
     */
    class func show(animated: Bool = true, forItem item: UIBarItem, withinSuperview superview: UIView? = nil, text: String, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil){
    ...
    /**
     ...
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil, text:  String, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil){
        
        let ev = TipView(text: text, preferences: preferences, delegate: delegate)
    ...
    /**
     ...
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forItem item: UIBarItem, withinSuperview superview: UIView? = nil, contentView: UIView, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil){
    ...
    /**
     ...
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil, contentView: UIView, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil){
        
        let ev = TipView(contentView: contentView, preferences: preferences, delegate: delegate)
        ev.show(animated: animated, forView: view, withinSuperview: superview)
    }
...
open class TipView: UIView {
    ...
    public convenience init (text: String, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil) {
        self.init(content: .text(text), preferences: preferences, delegate: delegate)
    }
    
    public convenience init (contentView: UIView, preferences: Preferences = TipView.globalPreferences, delegate: TipViewDelegate? = nil) {
...
extension UIView {
    ...
    fileprivate func hasSuperview(_ superview: UIView) -> Bool{
