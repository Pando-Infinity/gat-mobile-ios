...
class UserArticleVC: UIViewController {
    ...
    fileprivate func getData(){
        ...
        self.searchTextField.rx.text.subscribe(onNext: { (text) in
            self.param.accept(.init(userId: id, pageNum: 1, text: text ?? ""))
        }).disposed(by: self.disposeBag)
    ...
    fileprivate func showPostDetail(_ post:Post,style:OpenPostDetail){
        ...
        self.profileViewController?.navigationController?.pushViewController(postDetail, animated: true)
    }
...
extension UserArticleVC: UITableViewDelegate {
